...
from webapp.boq_excel import generate_boq_xlsx  # noqa: E402
from webapp.docgen import generate_quotation_docx  # noqa: E402

# Step 3 (Margin Calculator) lives in its own root folder; load its modules by
# adding that directory to sys.path (the folder name contains spaces).
...
async def get_sample_boq():
    extraction, _ = await asyncio.to_thread(_load_sample)
...
async def get_sample_spec():
    extraction, preview = await asyncio.to_thread(_load_sample)
    file_name = (
...
@app.post("/api/v1/boq/export")
async def export_boq(request: Request):
    """Generate a template-style BOQ .xlsx from the current BOQ and return it.

    The frontend POSTs the BOQ object it is currently showing (the extraction /
    BOQ dict with ``lineItems``). We render it into the customer BOQ template
    layout (DETAILS | ITEM | QTY ...) and stream the file back as a download.
    """
    try:
        boq = await request.json()
    except Exception as exc:
        raise HTTPException(400, f"Invalid JSON body: {exc}") from exc
    if not isinstance(boq, dict):
        raise HTTPException(400, "Request body must be a BOQ object.")

    case_ref = str(boq.get("caseReference") or boq.get("boqId") or "DRAFT")
    safe_ref = "".join(c for c in case_ref if c.isalnum() or c in ("-", "_")) or "DRAFT"
    out_dir = config.OUTPUT_DIR / "_boq_export"
    out_path = out_dir / f"Qualitrol_BOQ_{safe_ref}.xlsx"

    try:
        generated = await asyncio.to_thread(generate_boq_xlsx, boq, out_path)
    except Exception as exc:
        logging.error("BOQ export failed:\n%s", traceback.format_exc())
        raise HTTPException(500, f"BOQ export failed: {exc}") from exc
...
# --------------------------------------------------------------------------- #
# Step 3 — Margin calculator (compute / persist / export)
# --------------------------------------------------------------------------- #
