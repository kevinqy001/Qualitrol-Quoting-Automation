...
(function () {
  ...
  async function analyzeSelectedFiles() {
    ...
    // Live elapsed-time counter so a long (1-2 min) vision analysis clearly
    // looks like progress, not a hang — discouraging premature restarts.
    const progressText = $("#upload-progress-text");
    const baseMsg = "Extracting file text, analysing drawings with AI vision, and preparing evidence";
    const startedAt = Date.now();
    const progressTimer = setInterval(() => {
      if (progressText) {
        const secs = Math.round((Date.now() - startedAt) / 1000);
        progressText.textContent = `${baseMsg}… ${secs}s elapsed`;
      }
    }, 1000);
    ...
    try {
      ...
      if (err.name === "AbortError") {
        ...
        // "Failed to fetch" — the connection dropped (server stopped/restarted
        // mid-request, or a port conflict). The analysis itself can take 1-2 min.
        alert(
          "Connection to the server was lost during analysis.\n\n" +
            "This usually means the backend was stopped or restarted while the " +
            "(1–2 minute) analysis was still running, or a second 'python app.py' " +
            "hit the in-use port 8000.\n\n" +
            "Make sure exactly one server is running, keep this tab open, and try again."
        );
      } else {
      ...
      if (progressText) progressText.textContent = baseMsg + "…";
  ...
  $("#btn-approve-boq").addEventListener("click", () => {
    ...
    // Hand the reviewed BOQ to Step 3 and jump to the margin calculator.
    const filled = marginAutofillFromBoq();
    switchTab("margin");
    if (!filled) {
      $("#margin-status").textContent =
        "No BOQ line items to carry over — add lines manually or load a saved record.";
    }
  });

  async function exportBoqExcel() {
    const btn = $("#btn-export-boq");
    if (IS_STATIC) {
      alert("Excel export is only available when running the local backend (python app.py).");
      return;
    }
    if (!currentBoq || !(currentBoq.lineItems || []).length) {
      alert("No BOQ to export yet. Upload documents or load the sample first.");
      return;
    }
    ...
    try {
      const res = await fetch(`${API}/boq/export`, {
        method: "POST",
      ...
      const ref = (currentBoq.caseReference || currentBoq.boqId || "DRAFT")
        .toString()
        .replace(/[^A-Za-z0-9_-]/g, "");
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `Qualitrol_BOQ_${ref}.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
    } catch (err) {
      alert("BOQ export failed: " + err.message);
    } finally {
      btn.disabled = false;
  ...
  $("#btn-export-boq").addEventListener("click", exportBoqExcel);
  ...
  // ── Initial data load ──────────────────────────────────────────────────
  // Render the (empty) Configure & Quote table right away, then load the
  // product catalog first so the family/model pickers are ready quickly. The
  // heavier sample load runs afterwards and no longer blocks the catalog.
  renderMarginTable();
  marginLoadCatalog();
